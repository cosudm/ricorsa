...
export const POST = handle(async (req: Request) => {
  ...
  const rows = await db().insert(schema.connectors).values({ id: uid(), userId: user.id, name: name.slice(0, 60), serverName: serverNameFor('vault', existing.map(c => c.serverName)), preset: VAULT_PRESET, url: vaultMcpUrl(), authType: 'bearer', secret, spaceIds, status: 'new' }).returning();
