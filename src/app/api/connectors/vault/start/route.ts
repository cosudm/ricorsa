...
export const POST = handle(async (req: Request) => {
  ...
  const r = await vaultClient<{ challengeId: string; sent: boolean; delivery?: 'email' | 'staff'; devCode?: string }>('/connect/start', { email: b.data.email, externalUser: user.id });
  // delivery says how the code reaches the person: 'email', or 'staff' when the Vault has no mail provider yet and
  // a Vault administrator reads the code out from the staff console. In local development the Vault hands the code
  // back instead; never in production.
  return json({ challengeId: r.challengeId, email: b.data.email, delivery: r.delivery === 'staff' ? 'staff' : 'email', ...(process.env.NODE_ENV !== 'production' && r.devCode ? { devCode: r.devCode } : {}) });
});
