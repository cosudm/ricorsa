...
That is all the configuration. The first time a signed-in person opens the pricing page, the app creates its catalog product, the billing plans for each paid tier (prices from `src/lib/plans.ts`: monthly and yearly, each with the free trial and without it) and the webhook at `APP_BASE_URL/api/billing/paypal/webhook`, and keeps the ids in the `config` table (`src/lib/paypal-setup.ts`, keyed `essentials`, `essentials:annual`, `essentials:notrial`, `essentials:annual:notrial` and so on). If you would rather manage the standard plans by hand, set `PAYPAL_PLAN_ESSENTIALS`, `PAYPAL_PLAN_PROFESSIONAL`, `PAYPAL_PLAN_ENTERPRISE` (monthly), `PAYPAL_PLAN_*_ANNUAL` (yearly) and `PAYPAL_WEBHOOK_ID` and the app uses yours; `npm run paypal:setup` and `npm run paypal:webhook` are the same steps as scripts. PayPal plans are immutable, so a price change means new plans and new ids. Every webhook delivery is verified with PayPal's signature endpoint, recorded once, and applied by re-fetching the subscription so the account always reflects PayPal's current view.

Plans are reconciled with PayPal on every cold start (`src/lib/paypal-setup.ts`): a paid tier with no PayPal plan gets one, and a tier whose price changed gets a new one while the old id is kept as retired, so people subscribed at the old price keep their price and their access; the keys used before September 2026 (`pro`, `team`) resolve to Essentials and Professional in code and were rewritten in the database by migration 0010. How billing flows: the pricing page renders PayPal's subscription button for each paid plan in the chosen cycle with the user's id in `custom_id`. On approval the browser posts the subscription id to `/api/billing/paypal/subscribe`, the server fetches it from PayPal, checks the `custom_id`, works out the plan and cycle from the PayPal plan id (`planFromPaypalPlan`) and sets the plan, writing `billing_cycle` on the user and on the subscription row. A subscription that replaces a different one on the same account (a plan change, a switch from monthly to annual) cancels the old one at PayPal so nobody is billed twice, and events about a subscription the account has moved on from never touch the plan (`decideSubscription` in `src/lib/billing.ts`). Webhooks keep it in sync afterwards (activated, suspended, cancelled, expired, payment failed). Cancel from the Account page calls PayPal's cancel endpoint and re-syncs.
...
`src/lib/plans.ts` defines Essentials ($45 a month or $450 a year), Professional ($79 or $790) and Enterprise ($129 or $1,290; organizations license by the seat instead, 60 seats or more at $85 a seat), monthly and annual feature-identical, the annual price ten months' worth (`ANNUAL_MONTHS_FREE`, shown as two months free; the pricing page opens on annual with the monthly-equivalent price in whole dollars, rounded up, and an asterisk, `?cycle=monthly` opens on monthly). Prices moved from $25 / $55 / $85 in September 2026 so a heavy user at the metered ceilings still runs at a margin; people subscribed before keep their price on the retired PayPal plan ids. A first subscription starts with a free trial of `TRIAL_DAYS` (14) through PayPal on either cycle; the trial is one per account (`trialEligible`), so anyone who has held a subscription before subscribes to the plan's no-trial variant and bills from the first day. The Account page names the cycle and, after three months on monthly, suggests the switch and the best moment for it (just before renewal, since the monthly subscription is canceled when the annual one starts and PayPal does not prorate). Alongside those sits the free state an account has with no subscription (not offered as a plan): daily and monthly question caps, Research reports per month, which model tiers a plan may use, and how many Spaces. `assertQuota` runs before every answer and returns a 402 (upgrade) or 429 (limit) that the app turns into a friendly message with a link to pricing. The Build studio and Discover have ceilings of their own, since a version or an idea set costs far more than an answer: `buildsPerMonth` (30 on Professional, 100 on Enterprise) is checked by `assertBuildQuota` before the studio starts any work and counted in `usage.builds` when a version is written (a reply to a question in the studio counts as a question only); `ideaSetsPerMonth` (60 and 200) is checked by `assertIdeaQuota` before a new set is generated and counted in `usage.ideas` (a set served from the cache is free; at the ceiling the last set is served with a note). The messages name the ceiling and the reset, never a provider or a cost. Usage rows in the `usage` table carry token counts and an estimated cost so you can watch margin per user; the price constants near the bottom of `plans.ts` are the ones to update when list prices change.
...
Focus (Web, Academic, Technical, Legal, Writing, Math, Code) shapes the prompt and the search: Academic and Legal add site filters for scholarly and primary legal sources, Technical asks for specifications and standards, Code for documentation, Writing skips the web. Each answer starts with retrieval: one Brave query for a normal question (none for Writing focus), four to six planned queries plus a read of the top pages in Research mode. The merged, numbered results are pushed to the browser before the model starts and handed to the model as context after the question; it cites them with `[n]` markers that the client renders as chips. Connectors are offered to the model as functions; when it calls one, Ricorsa calls the MCP server and returns the result, until the model answers. Answers that hit the output limit continue in partial mode. Earlier answers are fed back into follow-ups with the markers stripped.

## Files: ask, find, highlight

A file attached to a question is read in full and, since version 1.2, also cut into passages that remember where they came from (`src/lib/passages.ts`: `[Page n]` for PDFs, `[Slide n]` for decks, `[Sheet: name]` with row ranges for workbooks, sentence groups for everything else). The passages that bear on the question (keyword scoring, with a floor of two per file so a broad question still has something to point at, and up to twelve when the question is a lookup such as "where does it mention") are numbered after the web sources under the label "Your files", each with its location and a link that opens the file in the viewer at that passage: `/app#/thread/<id>?turn=<turnId>&file=<fileId>&p=<page>&hl=<the sentence that answers>`. The model is told to cite them by number, so a claim from a file opens the file at the sentence it came from.

The viewer (`public/app/assets/viewer.js`, sandboxed) draws PDFs with pdf.js (pages as they scroll into view, each with a text layer), Word with docx-preview, workbooks as a grid, decks with PPTXjs, and the rest as text or code. Every kind has Find in this file (matches across the whole document, Enter and Shift+Enter walk them, the page is named for PDFs) and highlight (a citation's snippet is located in the rendered text, whitespace-insensitive, and the whole sentence is marked). Ask about this opens a pane beside the document: the question runs as a turn on the conversation the file belongs to, with the file attached again as the primary material, the answer streams in the pane, and a citation of the file lights the passage in place without leaving the viewer. A file still only attached to the composer hands the question to the composer instead.
