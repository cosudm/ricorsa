...
import { zAddress, zCustomerStatus, zOptEmail, zOptText, zPlan, zTags, zText, zCents, zBillingCycle } from './validate';
import { normalizePlanKey } from './plans';
import { isPaying, cycleOf } from './ricorsa';
...
export const CustomerInput = z.object({
  ...
  status: zCustomerStatus.optional(), plan: zPlan.optional(), billingCycle: zBillingCycle.nullable().optional(), mrrCents: zCents.optional(), currency: zText(3).optional(),
...
function toView(c: CustomerRow, extra: { contacts: number; openCents: number; invoices: number; license: { plan: string; billingCycle: string | null; endsAt: number | null; status: string } | null; trial: { plan: string; endsAt: number; status: string } | null; account: { plan: string; subscriptionStatus: string | null; billingCycle: string | null; lastSeenAt: number; email: string | null } | null }) {
  return {
    id: c.id, kind: c.kind, name: c.name, company: c.company, email: c.email, phone: c.phone, website: c.website, address: c.address, status: c.status, plan: c.plan === 'custom' ? 'custom' : normalizePlanKey(c.plan),
    billingCycle: c.billingCycle || null, mrrCents: c.mrrCents, currency: c.currency, source: c.source, ownerId: c.ownerId, tags: c.tags, notes: c.notes, ricorsaUserId: c.ricorsaUserId, custom: c.custom,
...
export async function listCustomers(ids?: string[]): Promise<CustomerView[]> {
  ...
  let accounts: Array<{ id: string; plan: string; subscriptionStatus: string | null; billingCycle: string | null; lastSeenAt: Date; email: string | null }> = [];
  if (linked.length) { try { accounts = await rdb().select({ id: ricorsa.rUsers.id, plan: ricorsa.rUsers.plan, subscriptionStatus: ricorsa.rUsers.subscriptionStatus, billingCycle: ricorsa.rUsers.billingCycle, lastSeenAt: ricorsa.rUsers.lastSeenAt, email: ricorsa.rUsers.email }).from(ricorsa.rUsers).where(inArray(ricorsa.rUsers.id, linked)); } catch (e) { console.warn('[customers] product database unavailable', String((e as Error)?.message || e)); } }
  // The billing cycle of a paying account is the truth about how the customer pays: the record follows it.
  for (const c of rows) {
    const acc = accounts.find(a => a.id === c.ricorsaUserId);
    if (acc && isPaying(acc)) { const cycle = cycleOf(acc); if (c.billingCycle !== cycle) { c.billingCycle = cycle; await d.update(schema.customers).set({ billingCycle: cycle }).where(eq(schema.customers.id, c.id)); } }
  }
  return rows.map(c => {
    ...
    return toView(c, {
      ...
      license: lic ? { plan: normalizePlanKey(lic.plan), billingCycle: lic.billingCycle || null, endsAt: lic.endsAt?.getTime() ?? null, status: lic.status } : null,
      trial: tr ? { plan: normalizePlanKey(tr.plan), endsAt: tr.endsAt.getTime(), status: tr.status } : null,
      account: acc ? { plan: normalizePlanKey(acc.plan), subscriptionStatus: acc.subscriptionStatus, billingCycle: isPaying(acc) ? cycleOf(acc) : null, lastSeenAt: acc.lastSeenAt.getTime(), email: acc.email } : null,
    });
  });
}
