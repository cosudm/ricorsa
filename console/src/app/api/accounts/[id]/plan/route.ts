...
/** POST /api/accounts/:id/plan — set the plan on a Ricorsa account directly (a license or trial by hand, or back to Free). */
export const POST = handle(async (req: Request, ctx: Ctx) => {
  ...
  await logActivity(me, 'ricorsa.grant', 'ricorsa', userId, `Set ${acc?.email || userId} to ${b.plan}${b.plan !== 'free' ? ` (${b.kind === 'trial' ? 'trial' : 'license'}${b.until ? ' until ' + new Date(b.until).toDateString() : ''})` : ''}`, { data: r as unknown as Record<string, unknown> });
