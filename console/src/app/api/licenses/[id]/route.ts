...
export const PATCH = handle(async (req: Request, ctx: Ctx) => {
  ...
  if (!row) return fail(404, 'No such license', 'not_found');
  const b = parse(LicensePatch, await readJson(req));
  const set: Record<string, unknown> = { updatedAt: new Date() };
  if (b.plan) set.plan = b.plan; if (b.billingCycle) set.billingCycle = b.billingCycle; if (b.seats) set.seats = b.seats; if (b.autoRenew !== undefined) set.autoRenew = b.autoRenew; if (b.notes !== undefined) set.notes = b.notes;
  ...
  await logActivity(me, 'license.' + (b.action || 'update'), 'license', id, `${what} the ${(b.plan || row.plan)} license of ${await customerName(row.customerId)}`, { customerId: row.customerId });
...
export const DELETE = handle(async (_req: Request, ctx: Ctx) => {
  ...
  await logActivity(me, 'license.delete', 'license', id, `Deleted a license of ${await customerName(row.customerId)}`, { customerId: row.customerId });
