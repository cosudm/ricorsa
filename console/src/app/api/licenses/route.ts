...
/** GET /api/licenses — every license with its customer, newest first. Expired dates are reported as they stand; the grid marks them. */
...
/** POST /api/licenses — issue a license and, by default, apply it to the linked Ricorsa account. */
export const POST = handle(async (req: Request) => {
  ...
  const id = uid();
  const startsAt = b.startsAt ? new Date(b.startsAt) : new Date(), endsAt = b.endsAt ? new Date(b.endsAt) : null;
  // A yearly term bills annually unless the form said otherwise; anything shorter or open-ended is monthly.
  const billingCycle = b.billingCycle || (endsAt && endsAt.getTime() - startsAt.getTime() >= 300 * 86400e3 ? 'annual' : 'monthly');
  await d.insert(schema.licenses).values({ id, customerId: b.customerId, key: newLicenseKey(), plan: b.plan, billingCycle, seats: b.seats || 1, status: 'active', startsAt, endsAt, autoRenew: !!b.autoRenew, notes: b.notes || '', createdBy: me.id });
  await logActivity(me, 'license.create', 'license', id, `Issued a ${planName(b.plan)} license (${billingCycle}) to ${await customerName(b.customerId)}${b.endsAt ? ' until ' + new Date(b.endsAt).toDateString() : ''}`, { customerId: b.customerId });
