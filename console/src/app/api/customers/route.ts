...
export const POST = handle(async (req: Request) => {
  ...
  await db().insert(schema.customers).values({
    ...
    address: b.address ?? null, status: b.status || 'lead', plan: b.plan === 'custom' ? 'custom' : normalizePlanKey(b.plan || 'free'), billingCycle: b.billingCycle ?? null, mrrCents: b.mrrCents ?? 0, currency: (b.currency || 'USD').toUpperCase(), source: b.source ?? null,
